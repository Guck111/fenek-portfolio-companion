...
- What it is: a strictly READ-ONLY companion that reads the user's portfolio across Trading 212, Interactive Brokers, on-chain crypto wallets (Solana, TON, Bitcoin, Litecoin, Dogecoin), and Bybit, and helps analyze it. It never places trades, moves funds, or gives buy/sell/rebalance advice — it surfaces data and neutral metrics; decisions stay with the user.
- What can be connected (ALL OPTIONAL — set up only the sources you have, in Settings → Extensions → Fenek Portfolio Companion):
  ...
  - Interactive Brokers — a READ-ONLY Flex Web Service token plus the Query ID of an Activity Flex Query (IBKR Client Portal → Settings → Account Settings → Flex Web Service). Configure the Flex Query to include Open Positions, Net Asset Value, Cash Report and Cash Transactions (and optionally Trades). Gives positions, account NAV and cash, dividends with withholding tax, cash transactions and trade history. Note: IBKR data is END-OF-DAY from the Flex statement, not a live intraday quote, and covers a single account.
  - Crypto wallets — paste your public wallet addresses into one field (any separator; the chain of each — Solana, TON, Bitcoin, Litecoin, Dogecoin — is detected automatically). Holdings are read keyless — no API key needed — valued in USD, plus open Jupiter limit orders on Solana.
...
export function createGettingStartedPrompt(): PromptBinding {
  return {
    prompt: {
      ...
      description:
        "Explains what Fenek Portfolio Companion does, which sources it supports (Trading 212, Interactive Brokers, crypto wallets — Solana/TON/Bitcoin/Litecoin/Dogecoin, Bybit incl. derivatives and Earn), and how to configure them. Needs no API keys — run it before setting anything up.",
    },
    handler: () =>
      Promise.resolve({
