...
- What it is: a strictly READ-ONLY companion that reads the user's portfolio across Trading 212, on-chain crypto wallets (Solana, TON, Bitcoin, Litecoin, Dogecoin), and Bybit, and helps analyze it. It never places trades, moves funds, or gives buy/sell/rebalance advice — it surfaces data and neutral metrics; decisions stay with the user.
...
export function createGettingStartedPrompt(): PromptBinding {
  return {
    prompt: {
      ...
      description:
        "Explains what Fenek Portfolio Companion does, which sources it supports (Trading 212, crypto wallets — Solana/TON/Bitcoin/Litecoin/Dogecoin, Bybit incl. derivatives and Earn), and how to configure them. Needs no API keys — run it before setting anything up.",
    },
    handler: () =>
      Promise.resolve({
